...
# Pour chaque joueur
for p in P:
    # Un joueur ne peut sortir que s'il est dans transferts
    if p not in transferts_10 and x[p][0] == 1:
        prob += x[p][10] == x[p][0]  # période 10
    if p not in transferts_15 and x[p][10] == 1:
        prob += x[p][15] == x[p][10] # période 15
...
for i in range(1, len(T)):
    ...
    for p in P:
        ...
        # Transfert des joueurs entrant dans l'équipe
        # Force la variable z[p][t] à 1 lorsqu’un joueur rejoint l’équipe
        prob += x[p][t] - x[p][t_1] <= z[p][t]

        # Un joueur ne peut pas entrer dans l'équipe si il en fait déjà partie
        prob += z[p][t] <= 1 - x[p][t_1]

    # c_augmentation[p] * z[p][t]             : si un joueur entre en période t, alors on retire au budget total son prix de vente (avec l'augmentation des prix)
    # c[p] * (x[p][t_1] - x[p][t] + z[p][t]   : - si un joueur entre en période t, alors z[p][t] = 1, donc x[p][t_1] = 0 et x[p][t] = 1. Au final, on obtient 0 
    #                                         : - si un joueur est déjà dans l'équipe, alors z[p][t] = 0, donc x[p][t_1] = 1 et x[p][t] = 1. Au final, on obtient 0
    #                                         : - si un joueur sort en période t, alors z[p][t] = 0 (le joueur était dans l'équipe), donc x[p][t_1] = 1 et x[p][t] = 0. Au final, on obtient 1
    prob += r[t] == r[t_1] - pulp.lpSum(c_augmentation[p] * z[p][t] for p in P) + pulp.lpSum(c[p] * (x[p][t_1] - x[p][t] + z[p][t]) for p in P)
    prob += r[t] >= 0
...
team = equipe_opt(15)
print(f"\nÉquipe optimale à la période t={15}")
for p in team:
    print(f"{p} | {points.get((p, 22), 0)}")

print(f"\n--- Entrées à la période {t} ---")
cout_achat = 0
for p in P:
    if pulp.value(z[p][t]) == 1:
        cout = c_augmentation[p]
        cout_achat += cout
        print(f"  - {p} (coût augmenté) : {cout}")

print(f"Coût total des entrées : {cout_achat}")
...
for p in P:
    if pulp.value(x[p][t_1]) == 1 and pulp.value(x[p][t]) == 0:
        gain = c[p]
        gain_vente += gain
        print(f"  - {p} (prix de vente) : {gain}")

print(f"Revenu total des ventes : {gain_vente}")

print("\n================ ÉQUIPES OPTIMALES PAR PÉRIODE ================")
...
for t in T:
    ...
    for p in team:
        ...
        # points des étapes correspondant à la période
        for e in E:
            if period(e) == t:
                pts += points.get((p, e), 0)

        # bonus final pour la dernière période
        if t == 15:
            pts += points.get((p, E_final), 0)

        total_points += pts
        cout_total += c[p]

        print(f"{p:25} | points: {pts:4} | coût: {c[p]}")

    print("\nPoints totaux période :", total_points)
    print("Coût total équipe :", cout_total)
    score_final += total_points
